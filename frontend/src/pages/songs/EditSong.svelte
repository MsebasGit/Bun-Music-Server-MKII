<script lang="ts">
    ...
    async function handleUpdateSong() {
        ...
        try {
            const detailsToUpdate: { [key: string]: any } = {};
            const initialGenres = Array.isArray(song.genre) ? song.genre : [song.genre].filter(g => g);

            // Compare and add changed fields to the update object
            if (title !== song.title) {
                detailsToUpdate.title = title;
            }
            if (language !== song.language) {
                detailsToUpdate.language = language;
            }
            if (parsedGenres.sort().join(',') !== initialGenres.sort().join(',')) {
                detailsToUpdate.genres = parsedGenres;
            }

            // 1. Update text-based fields ONLY if they have changed
            if (Object.keys(detailsToUpdate).length > 0) {
                const response = await songApi.update(song.id_song, detailsToUpdate);
                if (!response.success) {
